...
export const Footer = ({ settings }: FooterProps) => {
  ...
  return (
    <footer className="footer section" data-padding="large">
      <div className="wrapper">
        <div className="footer__toys">
          <img src="/images/eit-toys-left.png" alt="" className="footer__toy" loading="lazy" />
          <img src="/images/eit-toys-right.png" alt="" className="footer__toy" loading="lazy" />
        </div>

        {settings?.logo && (
          <div className="footer__logo">
            <img src={settings.logo} alt={settings?.siteTitle || ""} loading="lazy" />
          </div>
        )}
        ...
        {social.length > 0 && (
          <div className="footer__social">
            {social.map((item, index) => (
              <a key={index} href={item?.url || "#"} className="footer__social-link" target="_blank" rel="noopener noreferrer" aria-label={item?.label || ""}>
                {item?.icon ? socialIcons[item.icon] || item.label : item?.label}
              </a>
            ))}
          </div>
        )}
